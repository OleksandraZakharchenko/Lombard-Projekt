<template>
  <FadeTransition>
    <BaseCard>
      <BaseHeading variant="h2">Twoje dane</BaseHeading>
...
const validateForm = () => {
  ...
  if (!isNameInputValid || !isPhoneValid) {
    formError.name = isNameInputValid
      ? ""
      : "W polu muszą znaleźć się więcej niż liczba 2 liter";
    (formError.phone = isPhoneValid
      ? ""
      : "Numer telefonu to przedział od 10 do 20 cyfr"),
      (isFormValid = false);
  }
...
const inputs = computed(() => {
  return inputFields.filter((field) => {
    ...
    if (props.delivery.type === "Odbiór osobisty") {
      return field.name !== "Lokacja" && field.name !== "Adres";
    }
