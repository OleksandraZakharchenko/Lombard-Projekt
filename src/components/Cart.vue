  ...
  <section class="cart">
    <div class="cart__mobile">
      <div class="cart__grid">
        <label class="cart__title">Zdjęcie</label>
        <label class="cart__title">Tytuł</label>
        <label class="cart__title">Ilość</label>
        <label class="cart__title">Cena</label>
        <label class="cart__title">Cała kwota</label>
      </div>
    </div>
    ...
    <ul class="cart__list">
      <li class="cart__grid" v-for="product in cart" :key="product.id">
        ...
        <div class="cart__mobile">
          <span>Cena:</span>
          <p class="cart__item-text">{{ product.price }} PLN</p>
        </div>

        <div class="cart__mobile">
          <span>Cała kwota:</span>
    ...
    <div class="cart__totals-block">
      <span class="cart__totals-block-title">Całkowita ilość:</span>
      <p>{{ totalProductsAddedToCart }} szt.</p>
    </div>
    <div class="cart__totals-block">
      <span class="cart__totals-block-title">Cała kwota: </span>
      <p>{{ totalAmount }} PLN</p>
    </div>

    <div class="cart__totals-block" v-if="delivery && delivery.name">
      <span class="cart__totals-block-title">Koszt wysyłki:</span>
      <p>{{ !isFreeDelivery ? "" : "" }}{{ deliveryPrice }} PLN</p>
    </div>

    <div class="cart__totals-block" v-if="delivery && delivery.name">
      <span class="cart__totals-block-title">Całość:</span>
      <p>{{ total }} PLN</p>
    </div>
  </section>
</template>
...
const deliveryPrice = computed(() => {
  ...
  return isFreeDelivery.value ? "0" : props.delivery.price;
});
